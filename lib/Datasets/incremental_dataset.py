...
def get_incremental_dataset(parent_class, args):
    ...
    class IncrementalDatasetMultihead(parent_class):
        """
        Incremental dataset class. Inherits from a dataset parent class. Defines functions to split classes into
        separate sets, incrementing the current set and replacing previous sets with generative replay examples.
        ...
        def __init__(self, is_gpu, device, task_order, args):
            ...
            # List of training sets (needed for upper bound)
            self.mh_trainsets = []
            ...
            self.maps_target_head = []
            # List of dicts mapping from head to (global) target space (index = task)
            self.maps_head_target = []
            ...
            print("maps target head")
            print(self.maps_target_head)
            print("maps head target")
            print(self.maps_head_target)

            # Append trainset to list of trainsets if upper bound run
            if args.train_incremental_upper_bound:
                self.mh_trainsets.append(copy.deepcopy(self.trainset))
        ...
        def __get_initial_dataset(self):
            ...
            # Get the maps from an to head and target space
            target_to_head = {}
            head_to_target = {}
            for i in range(len(self.seen_tasks)):
                target_to_head[self.seen_tasks[i]] = i
                head_to_target[i] = self.seen_tasks[i]
            # Append them to their respective lists
            self.maps_target_head.append(target_to_head)
            self.maps_head_target.append(head_to_target)
        ...
        def increment_tasks(self, model, batch_size, workers, writer, save_path, is_gpu,
            ...
            if upper_bound_baseline:
                ...
                # Store new trainset if multiheaded upper bound run (need them to be separated)
                if args.train_incremental_upper_bound:
                    self.mh_trainsets.append(copy.deepcopy(torch.utils.data.ConcatDataset(new_trainsets)))
            ...
            # note that validation is always conducted on real data, while training can be done on generated samples.
            new_valsets = [self.valsets.pop(j) for j in sorted_new_tasks]
            #new_valsets.append(self.valset) # do not append the last valset to the current when using multihead
            self.valset = torch.utils.data.ConcatDataset(new_valsets)
            ...
            for i in range(len(new_tasks)):
                target_to_head[new_tasks[i]] = i
                head_to_target[i] = new_tasks[i]
            # Append them to their respective lists
            self.maps_target_head.append(target_to_head)
            self.maps_head_target.append(head_to_target)
            print(self.maps_target_head)
    ...
    class IncrementalSegmentationDataset(parent_class):
        """
        Incremental dataset class. Inherits from a dataset parent class. Defines functions to split classes into
        separate sets, incrementing the current set and replacing previous sets with generative replay examples.
